<template>
  <div
    class="flex flex-col gap-8 mt-4 mb-8 items-center lg:justify-between lg:h-[calc(100vh-4rem)] lg:-mt-4 lg:mb-0 lg:gap-20 lg:flex-row"
  >
    <img
      :src="'/images/pfp.jpg'"
      alt="profile picture"
      class="rounded-full h-64 w-64"
    />
    <div>
      <h2 class="text-4xl mb-4">
        <span class="text-cyan-500">Hello</span>, my name is Artur!
      </h2>
      <p class="text-xl">
        Welcome to my portfolio website. I am a software developer with 2 years
        of experience and a Masters Degree in Advanced Computer Science. Skilled
        in full stack development and game development, primarily using
        JavaScript/TypeScript and C#. I am currently seeking opportunities to
        apply my expertise in creating cutting-edge software applications.
      </p>
    </div>
  </div>
</template>
